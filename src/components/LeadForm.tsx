...
export function LeadForm({ compact = false, locale = "tr" }: { compact?: boolean; locale?: Locale }) {
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    data.locale=locale; data.sourcePath=window.location.pathname; data.referrer=document.referrer||"direct";
    try{
  ...
  return <form className={compact?"leadForm compact":"leadForm"} onSubmit={submit}>
    ...
    <div className="formGrid two">
      <label><span>{t.area}</span><input name="area" placeholder="Riva, Ortaçeşme, Ömerli…" required /></label>
    ...
    <div className="formGrid two">
      <label><span>{t.size}</span><input name="size" placeholder="850 m² / 140 m²" required /></label>
      <label><span>{t.parcel}</span><input name="parcel" placeholder="123 / 45" /></label>
    </div>
    <label><span>{t.price}</span><input name="askingPrice" placeholder="12.500.000 TL" /></label>
    <div className="formGrid two">
    ...
    <label><span>{t.note}</span><textarea name="note" placeholder="Yol, bina yaşı, kiracı, özel durum…" /></label>
    <label className="consent"><input type="checkbox" name="noticeRead" value="yes" required /><span><a href={localeHref(locale,"#privacy")}>{t.consent}</a></span></label>
